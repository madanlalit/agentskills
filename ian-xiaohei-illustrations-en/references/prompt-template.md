...
Generate one image per illustration. Substitute the variables for the current article. Never stitch multiple illustrations into a single image.

```text
Generate one standalone 16:9 horizontal English article illustration.

Visual DNA:
Pure white background. Minimalist black hand-drawn line art. Slightly wobbly pen lines. Lots of empty white space. Sparse red/orange/blue handwritten English annotations. Clean absurd product-sketch feeling. No gradients, no shadows, no paper texture, no complex background, no commercial vector style, no slide-deck infographic look, no cute mascot poster, no children's illustration, no realistic UI.
...
Core idea:
{the one core meaning this image must express}

Composition:
{specific scene: where Xiaohei is, what Xiaohei is doing, the main object, how information flows}
...
Color use:
Black for main line art and Xiaohei. Orange for main flow / path / arrows. Red only for key warnings / problems / results. Blue only for secondary notes, feedback, or system state.
